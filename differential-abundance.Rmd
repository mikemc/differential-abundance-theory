...
<!-- 0.4 is 1 / 2.6? Is there a reason why 0.4 is the canonical FD rather than 2.6 -->
...
<!-- I was initially confused by calling 0.44 a decrease in magnitude as compared with 0.15 as |0.44| > |0.15|, but see now that 0.15 is a larger multiplicative factor e.g. so that |log(0.44)| < |log(0.15)| -->
...
<!-- Again the sign error is on the log scale (unsure if being more explicit about this is helpful) -->
...
Equation \@ref(eq:density-prop-error) indicates that the FE in this measurement equals the (constant) species efficiency multiplied by ${B^{\mtot (a)}}/{\bar B^{(a)}}$, the ratio of the mean efficiency of the total-abundance measurement to that of the MGS measurement.
...
\end{align}
can be used to relate a microbial abundance variable $y$ to a _covariate_ $x$ and the _residual (or unexplained) error_ $\varepsilon$.
...
For each species $i$, we set $y^{(a)}$ in Equation \@ref(eq:regression) equal to $\log \tilde A_{i}^{(a)}$, and use either Ordinary Least Squares (OLS) or Maximum Likelihood Estimation (MLE) to estimate the coefficients $\alpha$ and $\beta$.
