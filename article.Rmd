...
- On whether MGS can be quantitative: Is there a reference that explains the disagreement? Perhaps this question hasn't been explicitly addressed in the literature. I was gesturing towards a tension where on one hand there are many papers on the problems with community sequencing and on the other hand many papers (often by the same authors, and including us) are using it to make quantitative inferences. KL suggests Mallick et al., 2017 DOI 10.1186/s13059-017-1359-z; Or Barlow et al. 2020 https://doi.org/10.1038/s41467-020-16224-6 as possible references.
...
- Amy points to rao2021mult as another good ref for the above
...
- This version skips over the effect of bias on MGS measurement, leaving that for the intro, and goes right to the effect on FCs. I think this is a mistake; I'd rather walk the reader through the logic starting with the effect on one sample.
- To accompany this, I'd like a new first figure, that illustrates the effect of competition. Essentially like Figure 2 of the eLife paper; perhaps using the vaginal mb example.


```{r vaginal-example, cache = TRUE, fig.cap = '(ref:vaginal-example)'}
svg_path <- here::here(
  "notebook/_posts/2021-05-10-vaginal-microbiome-example",
  "vaginal-microbiome-example_files/figure-html5",
  "vaginal-example-proportions-timepoints-1.svg"
)
...
\end{align}
where $\text{mean efficiency}(s)=\sum_{j}\text{actual}_j(s)\cdot \text{efficiency}_j$ in the denominator is the average efficiency of all taxa in that sample.
Intuitively, Equation \@ref(eq:error) says that the taxon will be over-represented by the extent to which its measurement efficiency is greater than the average efficiency of cells in the sample.
...
- When revising the above paragraph, pay attention to making clear that altough we are imposing competition via normalization to proportions, the experiment itself imposes competition on the absolute intermediates like DNA concentration and sequenced fragments.
...
- Comment from AW indicated that it was hard to follow the shift from describing the hypothesized absolute abundance regression model to describing the effect of bias on the fitted model. It could help to spell out this transition and make clear we're back to discussing the situation where the absolute abundance measurements are observed (biased) relative abundance measurements multiplied by some observation of the total.
- Could be some potential for confusion in that the text notes we can get magnitude and sign errors for different taxa in the sample samples, but the figure shows a magnitude and sign error in two different sample sets on the same taxon. It could therefore be a better illustration to use two different taxa , both on the Eastern genotypes; We'll get a sign error for Penicillium but a magnitude error for any taxon that decreases more.
...
> Might ratio-based analysis also be used to overcome the effects of bias in differential absolute abundance? In fact, some methods for determining absolute abundance are based on ratios among taxa in the MGS measurement instead of proportions. Rather than using a measurement of total abundance, these methods require determining the abundance of one or more reference taxa.

This needs to be revised to make clear the novelty of using non-spiked taxa as a reference; could help to replace "In fact, some methods" with clearer language.
Also make clear that some studies using reference taxa are using proportion-based methods.
Also, cite the appendix that gives an overview of AA methods, and explicitly state distinction between proportion-based and ratio-based methods.

Could help to change to:

- Ratio-based analysis can also be used to overcome the effects of bias in differential absolute abundance.
- /Make clear that the above ratio equation applies for relative and absolute observed or actual abudances (since totals cancel)/
- Suppose we knew the true absolute abundance of a reference taxon $r$; then we might estimate the absolute abundance of taxon $i$ from
  + Give the estimation equation
  + Explain the effect of taxonomic bias - get correct FCs
- It remains to determine the AA of one or more reference taxa.
  + Some existing AA methods involve using spike ins of extraneous taxa
    - discuss spike-in approach
    - often stated that bias a problem for spike-ins; however we see that it only affects the baseline and not the fold changes
      + this first part could be shunted to a discussion
    - most/all papers on spike-ins suppose the spike-in must be added in a known fixed amount.
    - however, the above argument indicates that it isn't required that the reference taxon is extraneous or has a fixed abundance; what is important is that we know the correct abundance up to a fixed multiplicative error
    - hence it can be a native taxon we've measured using a targeted method; this method can be biased as long as it has a fixed multiplicative error (unlike MGS bias.)

Need to consider if this is too detailed for the main text.
Might want to make this section of the appendix particularly clear if can't put this in the main text.


Key equations, to show side-by-side: Proportion-based and ratio-based AA inference.
