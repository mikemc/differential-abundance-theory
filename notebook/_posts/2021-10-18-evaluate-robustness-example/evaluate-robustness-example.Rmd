...
First, we load the relevant libraries and our data
...
library(simulator)
cal <- readRDS("callahan2017repl_phyloseq_clean.rds")
```
...
Now that we have all of the pieces, we can run them together as follows. Note that we are only going to run 4 iterations for now, but we have commented out code below showing you how to run 100 draws across 4 cores:
...
Here's how the estimated coefficient on preterm birth changes under different draws from a distribution given by $\text{efficiency}_{i}} \overset{iid}{\sim} LogNormal(0, \sigma^2_e)$, and how the p-value for testing the null hypothesis that the coefficient on preterm birth is zero:
