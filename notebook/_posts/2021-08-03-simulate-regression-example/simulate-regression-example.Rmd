...
Can see that the standard error is generally increased in the measured density estimates except for Species 9, whose residual variation we expect to be correlated with the log mean efficiency and hence negatively correlated with the measurement error.
...
aug <- lm_fits %>%
  ...
  ungroup %>%
  left_join(aug_mean_eff, by = c(".sample", "x"), suffix = c('.dens', '.mean_eff'))
```
...
cov_actual_with_mean_eff <- aug %>%
  ...
  with_groups(.otu, summarize,
    cov = cov(.resid.dens, .resid.mean_eff)
  ) %>%
  print
```
