...
A common application of this reference-normalization approach involves adding a 'spike-in' in a known (and typically constant) abundance across samples (@stammler2016adju, @ji2019quan, @tkacz2018abso, @harrison2021theq, @rao2021mult).

Another approach is to use naturally-occurring species that are thought to be constant across samples; we refer to such species as _housekeeping species_ by analogy with the housekeeping genes used for absolute-abundance conversion in gene-expression studies (@silver2006sele).
...
Some have expressed the need to minimize taxonomic bias between the reference species and the native species being normalized against it (see, for example, @harrison2021theq's recommendations for spike-in experiments).
Our results show that accurate DA results can be obtained so long as this bias is approximately constant across samples, a much weaker condition.

Our formulation of the reference species approach shows that there is no need for the reference species to have a constant abundance, so long as its abundance is known.
This observation suggests a new approach to performing reference-normalization, using targeted measurements of the absolute abundance of one or more naturally occurring species.
...
The most common form of targeted measurement involves using quantitative or digital to measure the concentration of a marker-gene in the extracted DNA.
It is also possible to directly measure cell concentration by performing digital PCR prior to DNA extraction (@morella2018rapi), flow cytometry with species-specific florescent probes, or CFU counting on selective media.
...
To demonstrate the ability for reference calibration to provide accurate FD measurements in the presence of significant taxonomic bias, we treated the abundance of one species (_Lactobacillus crispatus_) in the @brooks2015thet mock community data as known, and used it to calibrate the abundances of all species. 
