...
Our case studies explore a limited range of possibilities; however, we can begin to form some general conclusions by placing them within the context of the three scenarios considered for LFC estimation described in Section \@ref(differential-abundance). 

We saw several cases where the mean efficiency is stable across samples, so that bias does not affect LFC estimates (Scenario 1).
...
In the vaginal case study, we also observed that the mean efficiency was relative stable across vaginal microbiomes that were dominated by the same species, despite substantial bias and large FCs in non-dominant species.
...
In several cases, the mean efficiency varied systematically with the condition of interest sufficient to cause substantial systematic error in LFC estimates (Scenario 3).
...
The mean efficiency cannot change more than the species proportions constituting it.
Therefore, the error in the estimated proportion FCs tend to be practically significant only for species whose FCs are substantially smaller in magnitude than the largest FC.
...
For certain applications, however, it will be important to remember that the FCs between individual samples remain unreliable.
...
## [Temp] Summary and conclusions

[first draft of new summary, prior to revising the DA theory section]

The effect that consistent taxonomic bias has on proportion-based DA analyses depends on the MGS protocol, the biological system, and the sample comparisons under investigation.
Our case studies explore a limited range of possibilities; however, when combined with the theoretical framework developed Section 3, they suggest some general conclusions. 

In the first scenario, the mean efficiency remains stable across samples despite large variation in the relative efficiencies among species and in species composition among samples.
In this case, the error caused by consistent taxonomic bias _will mostly cancel_ in cross-sample comparisons, and bias will not substantially affect the inferred FCs between sample pairs or between sample conditions.
We observed this scenario in the analysis of pre-infection fungal microbiome samples of Leopold and Busby (2020).
A stable mean efficiency within marine soil cores is consistent with our comparison of qPCR- and MGS-derived growth rates in the Lloyd et al.
(2020) experiment, though we lack the power for firm conclusions.
In the MOMS-PI study, we found that the mean efficiency was stable across vaginal microbiomes with the same dominant species, even as non-dominant species had large FCs.

In the second scenario, the mean efficiency varies substantially across samples but is not associated with the condition of interest.
In this case, the measured FCs between pairs of samples can be highly distorted; however, these errors effectively average out, leading to reliable FCs between sample conditions.
For example, the mean efficiency varied widely across vaginal microbiomes in the MOMS-PI study, but was only associated with one out of the three proinflammatory cytokines we considered.
For the other three, bias had a negligible impact on the inferred associations between species and cytokine level.

In the third scenario, the mean efficiency systematically varies with the condition of interest, causing a systematic error in the estimated FCs between sample conditions even when averaged over many samples.
Examples include the comparison of foliar fungal microbiomes pre- and post-infection, for which the mean efficiency increased post-infection, and the comparison of vaginal microbiomes with low versus high diversity in the MOMS-PI study, for which the mean efficiency was typically lower in high-diversity samples.
Our analysis of marine sediment communities is consistent with a systematic decline of the mean efficiency with burial time sufficient to substantially inflate the estimated growth rates of slowly changing species.
[TODO: Mention gut microbiome infection, RX examples.]
The mean efficiency cannot change more than the species proportions constituting it.
Therefore, the error in the estimated proportion FCs tend to be practically significant only for species whose FCs are substantially smaller in magnitude than the largest FC.

—

Under what conditions should we expect this problematic third scenario?
Our case studies suggest one prominent mechanism for causing systematic variation in the mean efficiency: the existence of one or more species with unusually high or low efficiencies that tend to dominate the community more often in one sample condition than another.
This mechanism was responsible for the negative effect of bias in our DA analyses of foliar fungal microbiomes following infection and of vaginal microbiomes with low versus high diversity.
In experimental systems where no one species frequently forms a large fraction of the community, systematic variation of the mean efficiency can still arise through the collective change of many species that are associated with the condition of interest.
We described a plausible instance in the marine-sediment case study, where increased burial time selects for lysis-resistant (and so lower efficiency) species.
